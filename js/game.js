(function () {
  ...
  function getResult() {
    ...
    var faceMassageScore = (g['массаж'] || 0) + (g['релакс'] || 0) + (g['тонус'] || 0) + (b['отёки'] || 0);
    var recoveryScore = (g['релакс'] || 0) + (g['час для себя'] || 0) + (g['массаж'] || 0);
    var toneCareScore = (g['тонус'] || 0) + (g['сияние'] || 0) + (g['увлажнение'] || 0) + (g['уход'] || 0);

    var heavyNegative =
      (b['стресс'] || 0) + (b['недосып'] || 0) + (b['усталость'] || 0) +
      (b['дедлайн'] || 0) + (b['нет времени'] || 0);

    if (negativeCount >= 3 || heavyNegative >= 3) {
      return buildResult('recovery');
    }
    ...
    if (negativeCount >= 2) {
      recoveryScore += negativeCount * 1.5;
    }

    var scores = [
      { key: 'faceCare', value: faceCareScore },
      { key: 'faceMassage', value: faceMassageScore },
      { key: 'recovery', value: recoveryScore * (negativeCount >= 1 ? 1.2 : 0.7) },
      { key: 'toneCare', value: toneCareScore }
    ];

    scores.sort(function (a, b) { return b.value - a.value; });

    if (totalGood === 0 && negativeCount === 0) {
      return buildResult('faceCare');
    }

    return buildResult(scores[0].key);
  }
